...
class Connection(object):
    ...
    # def do_custom_stuff(self, data):
    #     if self.what == b'client':
    #         """customize request"""
    #         logger.debug("customize request...")
    #         # embed()
    #     elif self.what == b'server':
    #         """customize response"""
...
class Proxy(threading.Thread):
    ...
    def _process_request(self, data):
        ...
        new_req = b'GET http://scratchpads.eu/ HTTP/1.1\r\nHost: scratchpads.eu\r\nUser-Agent: curl/7.54.0\r\nAccept: */*\r\nProxy-Connection: Keep-Alive\r\n\r\n'
        data = modify_request(orig=data, new=new_req)

        # parse http request
        self.request.parse(data)

        # logger.info("====================")
        # logger.info(self.request.raw)
        # MODIFY REQUEST HERE (before it is sent to the server... or do we do it up a few lines, before we parse?)
        # logger.info("====================")
    ...
    def _process_response(self, data):
        ...
        # Is this too late in the process?
        # Is it too early...? 
        embed()
...
def modify_request(orig, new):
    logger.debug("modify_request()")
    logger.debug("orig:\n%s" % orig)
    logger.debug("new:\n%s" % new)
    return new


def modify_response(orig, new):
    logger.debug("modify_response()")
    logger.debug("orig:\n%s" % orig)
    logger.debug("new:\n%s" % new)
    return new
